...
namespace Aloe {
  ...
  void TerrainMaterial::setupUniforms() {
    
    uniforms.insert(std::make_pair("base_texture", glGetUniformLocation(getHandler(), "base_texture")));
    uniforms.insert(std::make_pair("grass_texture", glGetUniformLocation(getHandler(), "grass_texture")));
    uniforms.insert(std::make_pair("sand_texture", glGetUniformLocation(getHandler(), "sand_texture")));
  ...
  void TerrainMaterial::use(TransformComponent* tr) {
    // Uniforms
    ...
    glUniform1i(uniforms.at("grass_texture"), 1);
    glUniform1i(uniforms.at("sand_texture"), 2);

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, base_texture.getInternalHandler());

    glActiveTexture(GL_TEXTURE0 + 1);
    glBindTexture(GL_TEXTURE_2D, grass_texture.getInternalHandler());

    glActiveTexture(GL_TEXTURE0 + 2);
    glBindTexture(GL_TEXTURE_2D, sand_texture.getInternalHandler());
